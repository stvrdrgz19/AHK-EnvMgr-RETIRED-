...
REM echo.
REM echo Restoring "%~5" to %DB3% . . .
REM echo.
REM SqlCmd -S %SqlServer% -U %Username% -P %Password% -Q "ALTER DATABASE %DB3% SET SINGLE_USER WITH ROLLBACK IMMEDIATE; RESTORE DATABASE %DB3% FROM DISK='%LocalFolder%\%~5\%DB3%.bak' WITH  FILE = 1,  NOUNLOAD,  REPLACE; ALTER DATABASE %DB3% SET MULTI_USER;"
